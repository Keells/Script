...
def debut():
    ...
    print("*******************************************************")
    update = os.system('sudo apt -qq update')
    print("*******************************************************")
    if update == 0:
...
def fin():
    ...
    print("*******************************************************")
    upgrade = os.system('sudo apt -qq upgrade -y')
    print("*******************************************************")
    if upgrade == 0:
...
def check():
    test = True
    check_files()
    installable = os.system("sudo apt-cache pkgnames |grep '^"+progInstall+"$'")
    if installable == 256:
        logging.error("Le paquet "+ progInstall +" n'existe pas, ou ne porte pas exactement ce nom")
    if installable == 0:
        os.system("sudo apt install "+progInstall)
